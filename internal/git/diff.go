...
func (r *Repository) GetChangedFiles(ctx context.Context, stagedOnly bool) (map[string]string, error) {
	...
	if stagedOnly {
		statusOpts.StagedOnly = true
	}
	statusOpts.IncludeRenames = true

	changes, err := r.GetStatus(ctx, statusOpts)
	if err != nil {
		return nil, err
	}
	...
	for _, change := range changes {
		...
		// For untracked files, we need to show the full content as a diff
		if change.ChangeType == ChangeUnknown {
			// For untracked files, show the content as an addition
			content, err := r.GetWorkingTreeContent(ctx, path)
			if err != nil {
				return nil, fmt.Errorf("failed to get content for %s: %v", path, err)
			}
